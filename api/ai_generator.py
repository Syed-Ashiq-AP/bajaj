...
class AIAnswerGenerator:
    ...
    def generate_answer(self, question: str, context: str, max_tokens: int = 150) -> Optional[str]:
        ...
        print(f"🤖 Generating answer for question: {question[:100]}...")
...
Answer (one sentence only):"""
        ...
        print(f"🔑 Using {len(self.api_keys)} API keys for load balancing")
        
        for attempt in range(self.max_retries):
            try:
                api_key = self.get_next_api_key()
                print(f"🔑 Attempt {attempt + 1}: Using API key ending in ...{api_key[-8:]}")
                ...
                if response.status_code == 200:
                    try:
                        ...
                        print(f"✅ Successful API response: {result}")
                    ...
                    print(f"❌ API request failed (attempt {attempt + 1}): HTTP {response.status_code}")
